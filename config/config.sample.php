 ...
 *
 * CDrive fork modifications: documented cdrive_egress_*, cdrive_proxy_*,
 * cdrive_app_proxy, announcements.feed_url opt-in and lookup_server opt-in.
 */
...
$CONFIG = [
	...
	/**
	 * CDrive egress firewall (Graphene-style, private by default).
	 *
	 * Every outbound HTTP request made through the server HTTP client is
	 * checked before any packet leaves the server:
	 *
	 * - ``cdrive_egress_denylist``: domains that are always blocked. An entry
	 *   blocks the domain itself AND all of its subdomains, e.g. ``'tracker.example'``
	 *   also blocks ``'a.tracker.example'``.
	 ...
	 *   calling app is in ``cdrive_egress_app_allowlist``. In denylist mode
	 *   only the denylist applies.
	 *
	 * App ids are lowercase, e.g. ``'dav'``, ``'theming'``, ``'weather_status'``;
	 * server core itself is ``'core'``.
	 *
	 * ``cdrive_egress_allow_self`` (default ``true``): requests to the instance
	 * itself (from ``overwrite.cli.url``, ``overwritehost`` and
	 ...
	 * allowed request (with response status, duration, proxy used) and every
	 * block (no response attached) for Administration -> CDrive.
	 * ``cdrive_egress_log_max`` (default ``500``, max ``5000``) caps entries.
	 * NOTE: logged URLs may contain query-string tokens; visible to admins.
	 */
	'cdrive_egress_mode' => 'allowlist',
	'cdrive_egress_denylist' => [
		//'tracker.example',
	],
	'cdrive_egress_allowed_hosts' => [
		//'updates.example.org',
	],
	'cdrive_egress_app_allowlist' => [
		//'dav',
	],

	/**
	 * CDrive per-app forced proxying (anti-tracking).
	 *
	 * - ``cdrive_proxy_list``: global list of HTTP/HTTPS proxy URLs, e.g.
	 *   ``['http://proxy1.internal:8080', 'http://user:pass@proxy2.internal:8080']``.
	 *   This is the pool an app's network settings draw from.
	 * - ``cdrive_app_proxy``: map of app id (lowercase) to proxy selection:
	 *   ``true`` or ``'auto'`` picks a random proxy from the global list,
	 *   a proxy URL string pins that app to that proxy. Every request of a
	 *   configured app that passed the egress firewall above is FORCED through
	 *   its proxy, so the destination never sees the server IP.
	 *   Example: ``['weather_status' => 'auto', 'dav' => 'http://proxy1.internal:8080']``.
	 *
	 * Failover: if a proxied request fails to connect, it is retried once
	 * through a different random proxy. A proxy that fails 5 times within 6
	 * hours is nuked (banned) automatically and never selected again until the
	 * admin clears the ban with
	 * ``occ config:app:delete cdrive_egress proxy_banned``.
	 * Successful requests reset a proxy's failure counter. If an app requires
	 * a proxy but none is available, its requests fail closed (no direct
	 * connection is ever made).
	 *
	 * Defaults to empty list / empty map (feature off).
	 */
	'cdrive_proxy_list' => [
	 ...
	 *
	 * CDrive: opt-in. The code default is ``''`` (empty, disabled) so no user
	 * data is ever sent to a lookup server unless the admin sets a URL here.
	 * The egress firewall additionally gates the traffic (allow the
	 * ``lookup_server_connector`` app or the lookup host).
	 *
	 * Defaults to ``https://lookup.nextcloud.com`` upstream; CDrive default ``''``.
	 */
	'lookup_server' => 'https://lookup.nextcloud.com',

	/**
	 * CDrive: announcement feed URL (opt-in).
	 *
	 * The announcements crawler only fetches when this is set to a feed base
	 * URL. Empty (default) means no request is ever made.
	 *
	 * Defaults to ``''`` (disabled).
	 */
	'announcements.feed_url' => '',
