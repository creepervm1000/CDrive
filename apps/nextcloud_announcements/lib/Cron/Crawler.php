 ...
 *
 * CDrive fork: announcement feed opt-in. The feed is only fetched when the
 * admin explicitly sets 'announcements.feed_url'; by default nothing is
 * fetched and no request leaves the server.
 */
...
final class Crawler extends TimedJob {
	...
	protected function run(mixed $argument): void {
		...
		// CDrive: announcements are opt-in. Without an explicitly
		// configured feed URL no request is made.
		if ($this->getFeedUrl() === '') {
			return;
		}
		try {
	...
	/**
	 * Feed base URL. CDrive default is empty (disabled); the admin can
	 * opt in by setting the 'announcements.feed_url' system value.
	 */
	private function getFeedUrl(): string {
		$url = $this->config->getSystemValueString('announcements.feed_url', '');
		if ($url === '') {
			return '';
		}
		return rtrim($url, '/') . '/';
	...
	protected function readFile(string $file): ?string {
		...
		$response = $client->get($this->getFeedUrl() . $file);
