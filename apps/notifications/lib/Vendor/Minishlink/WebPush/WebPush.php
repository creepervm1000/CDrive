...
use OCA\Notifications\Vendor\Http\Client\Exception\HttpException;
use OCA\Notifications\Vendor\Http\Client\HttpAsyncClient;
use OCA\Notifications\Vendor\Http\Discovery\HttpAsyncClientDiscovery;
use OCA\Notifications\Vendor\Http\Discovery\Psr17FactoryDiscovery;
use OCA\Notifications\Vendor\Http\Discovery\Psr18ClientDiscovery;
...
use OCA\Notifications\Vendor\Psr\Log\LoggerInterface;
class WebPush
{
     ...
     */
    public function __construct(array $auth = [], array $defaultOptions = [], ?ClientInterface $client = null, ?RequestFactoryInterface $requestFactory = null, ?StreamFactoryInterface $streamFactory = null, ?HttpAsyncClient $asyncClient = null, ?LoggerInterface $logger = null)
    {
        $this->logger = $logger;
    ...
    public function flushPooled(callable $callback, ?int $batchSize = null, ?int $requestConcurrency = null): void
    {
        ...
        foreach ($batches as $batch) {
            ...
            $promises = [];
            foreach ($requests as $request) {
                $promises[] = $this->asyncClient->sendAsyncRequest($request)->then(function (ResponseInterface $response) use ($callback, $request): void {
