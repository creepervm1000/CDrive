 ...
 *
 * CDrive fork: CDrive proxy-failover middleware registered below.
 */
...
class ClientService implements IClientService {
	...
	public function newClient(?callable $handler = null): IClient {
		...
		// CDrive: traffic log (allowed requests + responses) and retry
		// once through a fresh proxy when a forced per-app proxy fails
		// to connect (failures are counted towards nuking).
		$egressGuard = new CdriveEgressGuard($this->config, $this->logger);
		$stack->push($egressGuard->trafficLogger(), 'cdrive traffic log');
		$stack->push($egressGuard->retryWithFreshProxy(), 'cdrive proxy failover');
