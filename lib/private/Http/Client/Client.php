 ...
 *
 * CDrive fork: all requests pass the CDrive egress firewall
 * (CdriveEgressGuard) before leaving the server.
 */
...
class Client implements IClient {
	private ?CdriveEgressGuard $egressGuard = null;
	...
	private function buildRequestOptions(array $options): array {
		...
		// CDrive forced per-app proxy: overrides any other proxy setting.
		// Only reached for requests that already passed the egress firewall,
		// so a forced app can only proxy to allowed destinations.
		$egressGuard = $this->getEgressGuard();
		if ($egressGuard->hasForcedProxies()) {
			$forcedProxy = $egressGuard->getProxyForApp($egressGuard->detectCallingApp());
			if ($forcedProxy !== null) {
				$options[RequestOptions::PROXY] = ['http' => $forcedProxy, 'https' => $forcedProxy];
				$options['cdrive_proxy_used'] = $forcedProxy;
			}
		}
	...
	private function getEgressGuard(): CdriveEgressGuard {
		return $this->egressGuard ??= new CdriveEgressGuard($this->config, $this->logger);
	}
	...
	protected function preventLocalAddress(string $uri, array $options): void {
		// CDrive egress firewall: runs for every request (including
		// redirects and async calls, which all funnel through here).
		$this->getEgressGuard()->check($uri);
